...
interface HeaderProps {
  theme: 'light' | 'dark';
  toggleTheme: () => void;
  onLogoClick: () => void; // Callback for when the logo/title is clicked
}
    ...
    <header className="bg-crm-card dark:bg-crm-dm-card shadow-md sticky top-0 z-50 transition-colors duration-300">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-20">
          <button
            onClick={onLogoClick}
            className="flex items-center focus:outline-none focus:ring-2 focus:ring-crm-accent rounded-md p-1 -ml-1" // Added focus styling and padding
            aria-label="Go to The CRM Hub homepage"
          >
